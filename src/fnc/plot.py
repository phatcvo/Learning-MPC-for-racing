...
def plotClosedLoopLMPC(lmpc, map):
    ...
    for i in range(2, TotNumberIt):
        plt.plot(SS[i][0:LapTime[i], 4], SS[i][0:LapTime[i], 0], '-r')
    plt.ylabel('vx')
    plt.subplot(712)
    for i in range(2, TotNumberIt):
        plt.plot(SS[i][0:LapTime[i], 4], SS[i][0:LapTime[i], 1], '-b')
    plt.ylabel('vy')
    plt.subplot(713)
    for i in range(2, TotNumberIt):
        plt.plot(SS[i][0:LapTime[i], 4], SS[i][0:LapTime[i], 2], '-k')
    plt.ylabel('wz')
    plt.subplot(714)
    for i in range(2, TotNumberIt):
        plt.plot(SS[i][0:LapTime[i], 4], SS[i][0:LapTime[i], 3], '-c')
    plt.ylabel('epsi')
    plt.subplot(715)
    for i in range(2, TotNumberIt):
        plt.plot(SS[i][0:LapTime[i], 4], SS[i][0:LapTime[i], 5], '-g')
    plt.ylabel('ey')
    plt.subplot(716)
    for i in range(2, TotNumberIt):
        plt.plot(uSS[i][0:LapTime[i] - 1, 0], '-c')
    plt.ylabel('Steering')
    plt.subplot(717)
    for i in range(2, TotNumberIt):
        plt.plot(uSS[i][0:LapTime[i] - 1, 1], '-k')
    plt.ylabel('Acc')
    plt.xlabel("Time (s)")
...
def animation_xy(map, lmpc, it):
    ...
    xPred = []; yPred = []
    SSpoints, = ax.plot(SSpoints_x, SSpoints_y, 'sb', label="Safe Set",zorder=0)
...
def saveGif_xyResults(map, LMPCOpenLoopData, lmpc, it):
    ...
    SSpoints_x = []; SSpoints_y = []
    xPred = []; yPred = []
    SSpoints, = ax.plot(SSpoints_x, SSpoints_y, 'og', label="Safe Set",zorder=0)
    line, = ax.plot(xPred, yPred, '-or', label="Predicted Trajectory",zorder=1)
